...
class CurrentRecyclerViewAdapter(
    ...
    private val clickListener: (Hourly) -> Unit,
    ...
    inner class MViewHolder(private val view: View) : RecyclerView.ViewHolder(view) {
        fun bind(hourly: Hourly, clickListener: (Hourly) -> Unit) {
            ...
            itemBinding.tempTv.text = hourly.temp.toString()
            itemBinding.timeTv.text = SimpleDateFormat("MM-dd/HH:mm").format(Date(hourly.dt * 1000))
