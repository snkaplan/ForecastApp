...
class ForecastRecyclerViewAdapter(
    ...
    private val clickListener: (ForecastDaily) -> Unit,
    ...
    inner class MViewHolder(private val view: View) : RecyclerView.ViewHolder(view) {
        fun bind(daily: ForecastDaily, clickListener: (ForecastDaily) -> Unit) {
            val cal: Calendar = Calendar.getInstance().apply { timeInMillis = daily.timestamp * 1000 }
